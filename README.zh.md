...
- 给**每个阶段写一行"完成标准"**，并跑一份最终验收清单：主路径走过、持久化以重载为证、边界输入试过、控制台干净（含 404）、git 干净
- **先验证再声称**：功能的检查通过了才算完成，不是按钮有反应就算完成
- **诚实地部署**：先查你的授权——有授权就部署并验证公网网址；没有就交给你一步登录操作。绝不声称一个它没打开验证过的网址
- 每个模块做完后用 4 行讲解（做了什么 / 为什么这样做 / 你学到了什么 / 验证了什么），然后自动继续
