...
一个给 AI 助手（Codex、Claude Code、Cursor、DeepSeek Harness 等工具）用的 skill，它能把 AI 变成一位耐心的新手专属 Vibe Coding 教练。

当一个新手只说一句"我想做一个网站"，别的什么都没说时，这个 skill 会让 AI：

- 在写代码**之前**，先弄清楚目标和最小第一版
- 把项目拆成清晰的阶段
- 每次只带一个 5～20 分钟的小步骤
- 一边做一边教怎么写提示词，附带可直接复制的模板
- 想法太大时诚实地"说不"，帮你砍小范围
...
- 5 个可直接复制的提示词模板（规划项目、做一个功能、报错求助、解释代码、复盘）
- 一段完整示范对话
- 10 个新手练手项目点子（各带最小第一版）
- 一份人话版术语表
- 快捷指令：`/start`、`/breakdown`、`/next`、`/teach-prompt`、`/check`、`/error`、`/explain`、`/ideas`、`/retrospective`
...
加载了这个 skill 的 AI 就会像教练一样回复：先理清想法、定好最小第一版，再交给你一个小步骤。
