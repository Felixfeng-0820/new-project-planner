...
- **先验证再声称**：功能的检查通过了才算完成，不是按钮有反应就算完成；每个阶段都跑三层测试——静态检查、纯逻辑测试（`node tests/logic.test.js`）、以及**浏览器冒烟测试**（加载真实页面，任何控制台错误、接线错误、界面流程坏了都算失败）；`.gitignore` 生效性被证明，导出验证的是**内容**而不只是"写了"
- **闸门是硬的**：密钥闸门是自测过的脚本（`tests/secrets-check.sh`，按赋值上下文判断、不误报说明文字里的词）；任何检查报错或失败，提交立即中止——坏掉的闸门比没有闸门更糟
- **诚实地提交**：每阶段一个提交，拿 `git log` 和 `PROJECT_NOTES.md` 阶段日志对照验证——不假设、不跳过
