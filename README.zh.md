...
一个给 AI 助手（Codex、Claude Code、Cursor、DeepSeek Harness 等工具）用的 skill，它能把 AI 变成一位面向**大学生和学得快的新手**的 Vibe Coding 教练——目标不是做玩具，而是做出**真正上线、能写进简历的作品**。

当一个学生只说一句"我想做一个课程表网站"，别的什么都没说时，这个 skill 会让 AI：

- 在写代码**之前**，先弄清楚目标和最小第一版
- 把用户放进一条 6 级学习路径（HTML/CSS → JavaScript → git → 部署 → 数据与 API → 进阶）里定位，缺什么教什么
- 把项目拆成包含**真实工程实践**的阶段：第一天就用 git、尽早部署、需要时引入 API 和数据库
- 一次一小步地推进；用户跟得上就提速
- 一边做一边教提示词，附带 7 个可直接复制的模板
- 想法太大时诚实地"说不"
...
- 7 个可直接复制的提示词模板（规划项目、做功能、报错求助、解释代码、复盘、讲透概念、代码审查）
- 一段完整示范对话（以课程表网站为例）
- 10 个学生向项目点子（简历主页、GPA 计算器、校园二手平台、AI 学习助手……），各带最小第一版
- 一份 31 个词的人话版术语表
- 快捷指令：`/start`、`/breakdown`、`/next`、`/teach-prompt`、`/check`、`/error`、`/explain`、`/ideas`、`/teach`、`/review`、`/stack`、`/retrospective`
