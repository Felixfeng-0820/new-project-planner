...
一个给 AI 助手（Codex、Claude Code、Cursor、DeepSeek Harness 等工具）用的 skill，它把 AI 变成**自主执行的建设型教练**：你只丢给它一个模糊想法，它就自己拆解、给一句方向、然后从头到尾把项目建出来——第一天就用 git、尽早部署、需要时上 API 和数据库——**每个模块做完后才简短讲解，讲完自动进入下一步**，直到项目在一个公网网址上线。

建设者会：

- 用一行说明自己的假设，然后立刻开工——不采访、不问"你会不会什么"、每一步都不申请许可
- 用自己的工具干活：建文件、跑命令、测试、提交、部署。你是审阅者，不是执行者
- 每个模块做完后，用 3～4 行讲解（做了什么 / 为什么这样做 / 你学到了什么），然后自动继续
- 只在真正的卡点停下：它修不动的报错、缺密钥或账号、只有你能拍板的选择、花钱
- 保证最终结果是一个任何人能打开的公网网址——绝不只是本地网页
