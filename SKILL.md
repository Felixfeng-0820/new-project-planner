...
**Three tiers of tests — logic, browser, and wiring.** Phase 1 must produce all three: (a) `logic.js` — the core logic as pure functions, no DOM, so it can be tested without a browser; (b) `tests/logic.test.js` — Node asserts on the pure logic: a persistence round-trip against a fake storage object, and a corruption case asserting the backup is written and the warning is raised; (c) `tests/browser.test.mjs` — a browser smoke test of the REAL page, because pure tests cannot catch wiring errors (a missing global throws `ReferenceError` only in the browser) or CSS bugs (a `display:flex` rule overriding the `hidden` attribute). The smoke test must: start a local static server; load the real page; fail on ANY console error or page error; walk the core flow by clicking; save, reload, assert the state came back; corrupt the storage, assert the warning is visible AND the export produces a blob whose content equals the corrupt payload; assert no failed requests or 404 resources; assert default-hidden elements are actually hidden (computed visibility). Preferred tool: Playwright. If a real browser cannot be installed, fall back to a jsdom-based smoke test and say plainly what it does NOT cover (rendering and CSS). Never claim browser-verified without running a browser test.

**`tests/check.sh` runs everything, and the gates are hard.** `tests/check.sh` runs, in order: static checks (files exist, no missing references) → `.gitignore` effectiveness (`git check-ignore .env .env.local '*.key'` and `git ls-files`) → the secrets gate script → `node tests/logic.test.js` → the browser smoke test. Any step that errors or exits non-zero ABORTS the phase — fix the tooling, then re-run. A broken gate is worse than no gate: never commit past a failing check, and never continue after a gate command errors.

**The secrets gate is a script you test once, then trust.** In phase 1, write `tests/secrets-check.sh` (`set -euo pipefail`) implementing assignment-aware patterns: known prefixes (`sk-`, `ghp_`, `github_pat_`, `xox[baprs]-`, `AKIA[0-9A-Z]{16}`, `-----BEGIN ... PRIVATE KEY-----`); assignments with concrete values like `(api[_-]?key|token|password|secret)\s*[:=]\s*["'][^"'\s]{6,}`; secret FILE names (`.env`, `*.pem`, `*.key`, `id_rsa`). Immediately self-test it twice: a fake real-looking secret must be flagged, and a prose sentence containing the word "secret" must pass. Run it before every commit; if it errors or flags anything, the commit is aborted. A prose word is not a leak; a quoted value is.
...
**Secrets gate before every commit.** The gate is `tests/secrets-check.sh` (see above), self-tested in phase 1. Run it before every commit: if it errors or flags anything, ABORT the commit immediately, fix the gate or the leak, then re-run. If a secret ever reached git history: revoke it first, rotate it, remove the file, and record the incident in `PROJECT_NOTES.md`. Revoking comes before anything else.
...
5. **Git is clean and safe** — `git status` clean, ONE commit per phase verified against `git log` and the `PROJECT_NOTES.md` phase log, secrets gate self-tested and run on every commit (no false alarms on prose words, no `.env` tracked).
...
8. **Repeatable checks, including browser tests** — `tests/check.sh` runs static checks, the self-tested secrets gate, `node tests/logic.test.js`, AND the browser smoke test; all pass from a clean state; `.gitignore` effectiveness proven; the corruption case asserts the export CONTENT, not just the write; the browser test proves the page loads with zero console errors and the real UI works.
...
*[Phase 1: records the baseline, builds, writes tests/check.sh, tests/logic.test.js, tests/browser.test.mjs, tests/secrets-check.sh (self-tested twice), commits. No narration.]*

Phase 1 recap: we made the folder, the git repo, and a three-tier test suite: pure-logic tests, a browser smoke test, and a self-tested secrets gate. Why: pure tests cannot catch wiring errors — a missing global throws only in the browser. You learned: a commit is a save point. Verified: static checks pass, logic tests pass, the secrets gate self-test passes (fake key flagged, prose word not), `git status` clean.

*[Phase 2: the browser smoke test catches two bugs the logic tests could not — a `ReferenceError` because the module was not attached to the browser, and a CSS rule that showed a default-hidden warning. Both fixed.]*
